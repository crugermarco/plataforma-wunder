...
const sheetConnections = {
  ...
  stock: {
    scriptUrl: 'https://script.google.com/macros/s/AKfycbyMdODlBKvgdiMcek-OQk2fbwCiNIpd6Xg4pDwrrWDFarmayQi04dNSW3Wr1lyxuL-N/exec',
...
async function loadStockData(type) {
    try {
        ...
        const url = `${sheetConnections.stock.scriptUrl}?${params.toString()}`;
        const response = await fetch(url);
        const result = await response.json();
        
        if (result.success) {
            renderStockItems(containerId, result.data);
            
            if (lastUpdatedElement) {
                const updateTime = new Date(result.lastUpdated);
                lastUpdatedElement.textContent = `Actualizado: ${updateTime.toLocaleTimeString()}`;
            }
        } else {
            console.error(`Error al cargar datos de ${type}:`, result.message);
        }
    } catch (error) {
        console.error(`Error al cargar datos de ${type}:`, error);
    } finally {
        const refreshButton = document.getElementById(`refresh-${type}`);
        if (refreshButton) {
...
function renderStockItems(containerId, stockData) {
    ...
    stockData.forEach(item => {
        ...
        const labelDiv = document.createElement('div');
        labelDiv.className = 'stock-label';
        labelDiv.textContent = item.model;
...
function setupStockAutoRefresh() {
    ...
    stockIntervals.platos = setInterval(() => {
        loadStockData('platos');
    }, sheetConnections.stock.refreshInterval);
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('refresh-platos')?.addEventListener('click', () => {
        loadStockData('platos');
    });

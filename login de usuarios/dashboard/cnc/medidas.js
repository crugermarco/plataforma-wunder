...
function showInitialForm() {
    const content = `
        <form id="initialForm" onsubmit="handleInitialForm(event)">
            ...
            <div class="form-group">
                <label>MACHINE</label>
                <input type="text" id="machine" required placeholder="Ej: 5">
            </div>
            <div class="form-group">
                <label>PROGRAM</label>
            ...
            <div class="modal-actions">
                ...
                <button type="submit" class="next-btn">Buscar Referencias</button>
            </div>
        </form>
    `;
    ...
    const content = `
        <div class="form-group">
            ...
            <div class="reference-summary">
                ...
                <div class="reference-item">
                    <strong>PLATE THICKNESS:</strong> ${refValues.plateThickness1 || 'N/A'}
                </div>
                <div class="reference-item">
                    <strong>TRACK ROJO:</strong> ${refValues.trackRojo1 || 'N/A'}
                </div>
                <div class="reference-item">
                    <strong>TRACK AZUL:</strong> ${refValues.trackAzul1 || 'N/A'}
                </div>
                <div class="reference-item">
                    <strong>BALL VERDE:</strong> ${refValues.ballVerde1 || 'N/A'}
                </div>
                <div class="reference-item">
                    <strong>BALL AMARILLO:</strong> ${refValues.ballAmarillo1 || 'N/A'}
                </div>
                <div class="reference-item">
                    <strong>BALL AZUL CIELO1:</strong> ${refValues.ballAzulCielo11 || 'N/A'}
                </div>
                <div class="reference-item">
                    <strong>BALL AZUL CIELO2:</strong> ${refValues.ballAzulCielo21 || 'N/A'}
                </div>
            </div>
...
function showMeasurementForm() {
    ...
    const content = `
        <div class="form-group">
            ...
            <div class="hint">Valor de referencia: ${refValue !== undefined && refValue !== 'N/A' ? refValue : 'N/A'}</div>
...
function clearAllRows() {
    if (rows.length === 0) {
        alert('No hay filas para limpiar');
        return;
    }
    
    if (confirm(`¿Estás seguro de que quieres eliminar todas las ${rows.length} fila(s)?`)) {
        rows = [];
        renderRows();
        document.getElementById('sendBtn').disabled = true;
        showMessage('🗑️ Todas las filas han sido eliminadas', 'success');
    }
}
